...
test.describe('WordAhead Participant Flow E2E', () => {
  test('Walk through complete participant flow with Sequence B and TS format', async ({ page }) => {
    ...
    await page.locator('input[name="ac_mid"]').nth(6).dispatchEvent('click'); // value 7 (index 6)
    ...
    // Answer questions: 9 Block A + 1 Block C = 10 questions + 1 attention check = 11 total
    for (let i = 1; i <= 9; i++) {
    ...
    await page.locator('input[name="ac_late"]').nth(0).dispatchEvent('click'); // value 1 (index 0)
  ...
  test('Walk through complete participant flow with Sequence B and TF format', async ({ page }) => {
    ...
    // Answer questions: 9 Block A + 9 Block B = 18 questions + 1 attention check = 19 total
    for (let i = 1; i <= 9; i++) {
      await page.locator(`input[name="pt_a${i}"]`).nth(3).dispatchEvent('click'); // value 4 (index 3)
    }
    for (let i = 10; i <= 18; i++) {
    ...
    // Answer questions: 9 Block A = 9 total + 1 attention check = 10 total
    for (let i = 1; i <= 9; i++) {
      await page.locator(`input[name="pt_a${i}"]`).nth(4).dispatchEvent('click'); // value 5 (index 4)
    }
    await page.locator('input[name="ac_late"]').nth(0).dispatchEvent('click'); // value 1 (index 0)
